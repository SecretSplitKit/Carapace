...
impl TempDir {
    fn new(tag: &str) -> Self {
        ...
        let dir =
            std::env::temp_dir().join(format!("carapace-vault-{tag}-{}", u64::from_le_bytes(n)));
...
fn write_tree(root: &std::path::Path) -> Vec<(String, Vec<u8>)> {
    ...
    let files: Vec<(String, Vec<u8>)> = vec![
        ...
        (
            "sub/nested/c.dat".to_string(),
            b"small nested file".to_vec(),
        ),
...
fn full_roundtrip_memory_store() {
    ...
    assert!(
        big.chunks.len() > 1,
        "big file should span multiple FastCDC chunks"
    );
    ...
    assert_eq!(
        ...
        "opened manifest matches sealed input"
    );
...
fn envelope_seal_open_verify_and_digest() {
    ...
    assert_eq!(
        *blake3::hash(&ingest.envelope.to_bytes()).as_bytes(),
        ingest.digest
    );
