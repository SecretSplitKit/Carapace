...
fn card_a() -> ContactCard {
    ...
    let mut card = ContactCard {
        ...
        enc_pub: rep(0x05),
        nodes: vec![node],
        offers: Offers { storage_bytes: 10_737_418_240, relay: true, trustee: true },
        version: 7,
        by: [0; 32],
        sig: [0; 64],
    };
...
fn card_b() -> ContactCard {
    ...
    let mut card = ContactCard {
        ...
        enc_pub: rep(0x08),
        nodes: vec![node],
        offers: Offers { storage_bytes: 5_368_709_120, relay: false, trustee: true },
        version: 3,
        by: [0; 32],
        sig: [0; 64],
    };
...
#[test]
fn b8_1_hello() {
    let m = Hello { protocol: 1, card_version: 7, roles: 0b111 };
...
fn b8_2_ceremony_abort() {
    let user_a = sk(0x01);
    let mut m = CeremonyAbort { ceremony_id: rep(0xA0), by: [0; 32], sig: [0; 64] };
    m.sign(&user_a);
    assert_frame(
...
fn b8_6_friendship_end() {
    let node_a1 = sk(0x03);
    let mut m = FriendshipEnd { user: pk(&sk(0x02)), ts: T0, by: [0; 32], sig: [0; 64] };
    m.sign(&node_a1);
    assert_frame(
...
fn b8_8_friend_request() {
    let node_b1 = sk(0x04);
    let mut m = FriendRequest { token: rep(0xE0), card: card_b(), by: [0; 32], sig: [0; 64] };
    m.sign(&node_b1);
    assert_frame(
...
fn b8_9_friend_accept() {
    ...
    let mut m = FriendAccept { card: card_a(), friendship, by: [0; 32], sig: [0; 64] };
    m.sign(&node_a1);
    assert_frame(
...
fn b8_10_delete_request() {
    let node_a1 = sk(0x03);
    let mut m = DeleteRequest { scope: 0, vid: Some(rep(0xC0)), by: [0; 32], sig: [0; 64] };
    m.sign(&node_a1);
    assert_frame(
...
fn b8_11_delete_ack() {
    ...
    let mut req = DeleteRequest { scope: 0, vid: Some(rep(0xC0)), by: [0; 32], sig: [0; 64] };
    req.sign(&node_a1);
    let payload = &req.encode_frame()[4..];
    let reference = *blake3::hash(payload).as_bytes();
    let mut m = DeleteAck { reference, ts: T0, by: [0; 32], sig: [0; 64] };
    m.sign(&node_b1);
    assert_frame(
...
fn b8_12_manifest_offer() {
    let node_a1 = sk(0x03);
    let mut m = ManifestOffer { vid: rep(0xC0), epoch: 42, digest: rep(0xD0), by: [0; 32], sig: [0; 64] };
    m.sign(&node_a1);
    assert_frame(
...
fn b8_13_replica_invite() {
    let node_a1 = sk(0x03);
    let mut m = ReplicaInvite { vid: rep(0xC0), epoch: 42, approx_bytes: 1_073_741_824, by: [0; 32], sig: [0; 64] };
    m.sign(&node_a1);
    assert_frame(
...
fn b8_14_replica_accept() {
    let node_b1 = sk(0x04);
    let mut m = ReplicaAccept { vid: rep(0xC0), quota_bytes: 2_147_483_648, by: [0; 32], sig: [0; 64] };
    m.sign(&node_b1);
    assert_frame(
...
fn b8_15_share_grant() {
    ...
    let mut m = ShareGrant {
        ...
        refs: vec![AnnounceRef { vid: rep(0xC0), epoch: 42, digest: rep(0xD0) }],
        by: [0; 32],
        sig: [0; 64],
    };
    m.sign(&node_a1);
...
fn b8_16_share_attest_challenge() {
    let node_a1 = sk(0x03);
    let mut m = ShareAttestChallenge { subject: pk(&sk(0x01)), rsid: 0x02C9, nonce: rep(0xB0), by: [0; 32], sig: [0; 64] };
    m.sign(&node_a1);
    assert_frame(
...
fn b8_17_share_destroy() {
    let node_a1 = sk(0x03);
    let mut m = ShareDestroy { subject: pk(&sk(0x01)), rsid: 0x02C9, by: [0; 32], sig: [0; 64] };
    m.sign(&node_a1);
    assert_frame(
...
fn b8_18_share_destroy_ack() {
    let node_b1 = sk(0x04);
    let mut m = ShareDestroyAck { subject: pk(&sk(0x01)), rsid: 0x02C9, ts: T0, by: [0; 32], sig: [0; 64] };
    m.sign(&node_b1);
    assert_frame(
...
fn b8_19_file_grant() {
    ...
    let mut m = FileGrant {
        ...
        sealed: vec![Sealed { to: pk(&sk(0x02)), ct: vec![0xEE; 48] }],
        by: [0; 32],
        sig: [0; 64],
    };
    m.sign(&node_a1);
...
fn b8_20_audit_notice() {
    let node_a1 = sk(0x03);
    let mut m = AuditNotice { vid: rep(0xC0), code: 1, by: [0; 32], sig: [0; 64] };
    m.sign(&node_a1);
    assert_frame(
...
fn b8_22_ceremony_approve() {
    let node_b1 = sk(0x04);
    let mut m = CeremonyApprove { ceremony_id: rep(0xA0), ts: T0 + 3600, by: [0; 32], sig: [0; 64] };
    m.sign(&node_b1);
    assert_frame(
...
fn b8_23_ceremony_share() {
    let node_b1 = sk(0x04);
    let mut m = CeremonyShare { ceremony_id: rep(0xA0), sealed: vec![0xEE; 48], by: [0; 32], sig: [0; 64] };
    m.sign(&node_b1);
    assert_frame(
...
fn b8_24_grant_body_doc() {
    let g = GrantBody {
        files: vec![GrantFile {
            ...
            file_hash: rep(0xAA),
            size: 1234,
            chunks: vec![GrantChunk { chunk_id: rep(0xC1), chunk_key: rep(0x77), nonce: rep(0x88), len: 1234 }],
        }],
    };
    assert_eq!(
...
fn reject_unsorted_keys() {
    ...
    assert_eq!(decode(&[0xa2, 0x01, 0x07, 0x00, 0x01]), Err(Error::UnsortedMapKeys));
    // duplicate key 0 also caught (equal, not strictly greater).
    assert_eq!(decode(&[0xa2, 0x00, 0x01, 0x00, 0x02]), Err(Error::UnsortedMapKeys));
}
...
fn reject_non_shortest_int() {
    ...
    assert_eq!(decode(&[0x1b, 0, 0, 0, 0, 0, 0, 0, 0]), Err(Error::NonCanonicalInt));
}
...
fn reject_sig_over_nondeterministic_encoding() {
    ...
    user_a.verifying_key().verify_strict(&nc, &ed25519_dalek::Signature::from_bytes(&bad_sig)).unwrap();

    let m = CeremonyAbort { ceremony_id: cid, by, sig: bad_sig };
...
fn frame_rejects_trailing_bytes() {
    let m = Hello { protocol: 1, card_version: 7, roles: 7 };
...
fn wrong_type_rejected() {
    ...
    assert_eq!(
        ...
        Err(Error::WrongType { expected: 21, got: 1 })
    );
}
...
fn friend_request_rejects_unsigned_embedded_card() {
    ...
    let mut m = FriendRequest { token: rep(0xE0), card, by: [0; 32], sig: [0; 64] };
