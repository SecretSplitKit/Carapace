//! §11 filesystem watcher: a local change under a published vault's source dir re-ingests it
//! (epoch++), Dropbox-style. Runs against a real `notify` backend but every wait is capped by
//! a timeout, so a watcher that stops firing fails fast instead of hanging.
...
/// Ceiling on waiting for the watcher to observe a change and re-publish (fs latency +
/// debounce + re-ingest); exceeding it means the watcher never fired.
...
/// Give the freshly-armed watcher a beat to settle so the write lands as a distinct observed
/// event (avoids racing setup on slow CI).
